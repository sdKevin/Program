function Fig2b_Left_Plotting(ETrc_Year , Met_Year)
% Drawing AI calculated by PM_RC_CO2_Yang
%% Data Bias Correction and Smooth
for ii = [2 : length(ETrc_Year)-1 , 1]
    for iii = 1 : size(ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang,1)
        
        % HadGEM3-GC31-LL does not have ssp370
        if ii == 4 && iii>=16
            % ETrc_Year Bias Correction
            ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) = ...
                ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) - ...
                (mean(ETrc_Year(1).ETrc_Year.PM_RC_CO2_Yang(iii+1,99:end)) - mean(ETrc_Year(6).ETrc_Year.PM_RC_CO2_Yang));
            % ETrc_Year Smooth
            ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) = ...
                smooth(ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) , 7);
            ...
            ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) = ...
                ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) - ...
                (mean(ETrc_Year(1).ETrc_Year.PM_RC_CO2_Yang(iii,99:end)) - mean(ETrc_Year(6).ETrc_Year.PM_RC_CO2_Yang));
            % ETrc_Year Smooth
            ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) = ...
                smooth(ETrc_Year(ii).ETrc_Year.PM_RC_CO2_Yang(iii,:) , 7);
            % Met_Year Bias Correction
...
YlimRange_AI = [-0.02,0.02] ;YTickRange_AI = [-0.01:0.01:0.09]; YTickLabel_AI = {'';'0';'';'0.02';'';'0.04';'';'0.06';'';'0.08';''};
...
% Historical AI calculated by PM-RC-Yang
...
% ScenarioMIP AI calculated by PM-RC-Yang
% pr kg/(m2s), ETrc mm/day
for i_ssp = [5,4,3,2]
    if i_ssp == 4
        AI_Historical = (Met_Year(1).Met_Year.pr .* 1000.*31536000./997) ./ ...
...
plot(0,'Color',RGB_Historical_Line./254,'Linewidth',3.5);hold on;
for i_ssp = [1,2,3,4]
