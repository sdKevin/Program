function Fig4_Timeseries_nat_Intensity(Met_Drought_Intensity_Year , Met_Drought_nat_Intensity_Year)
...
CD_ssp = Met_Drought_Intensity_Year(3).DroughtIntensity_Year.CompoundDrought;
...
h_ssp = fill([[2014:2100]';flipud([2014:2100]')],...
    ...
    RGB_ALL_Shade./255,'EdgeAlpha',0,'FaceAlpha',0.5); hold on;
clearvars -except Met_Drought_Intensity_Year Met_Drought_nat_Intensity_Year RGB_ALL_Shade RGB_ALL_Line RGB_NAT_Shade RGB_NAT_Line XlimRange YlimRange_CD YTickRange_CD
...
CD_piControl = Met_Drought_nat_Intensity_Year(3).DroughtIntensity_Year.CompoundDrought;
...
try
    ...
    Ensemble_Mean_CD_Historical = nanmean(CD_Historical([4,12,15,16,21],:))'; % Selecting 5 ESMs
    c95_CD_Historical = (std(CD_Historical([4,12,15,16,21],:))./sqrt(size(CD_Historical([4,12,15,16,21],:),1))) .* 1.96; % 95% confidence interval
    text(1951,0.066,strcat( '[' , num2str(mean(Ensemble_Mean_CD_Historical(102:151)),3),'¡À' ,...
...
try
    ...
    text(2051,0.066,strcat( '[' , num2str(mean(Ensemble_Mean_CD_ssp245(37:86)),3),'¡À' ,...
...
try
    ...
    text(2001,0.066,strcat( '[' , num2str(mean(Ensemble_Mean_CD_ssp245(152:201)),3),'¡À' ,...
