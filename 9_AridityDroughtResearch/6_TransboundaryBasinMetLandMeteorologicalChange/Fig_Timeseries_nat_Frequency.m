...
for ii = 1 : size(Met_Drought_Frequency_Year,2)
    ...
    if ii<4
        Met_Drought_nat_Frequency_Year(ii).DroughtFrequency_Year.CompoundDrought = Met_Drought_nat_Frequency_Year(ii).DroughtFrequency_Year.CompoundDrought .* 10;
    end
end
...
XlimRange = [1950,2100];
YlimRange_CD = [0.5,4.5]; YTickRange_CD = [0.5:1:4.5];
...
CD_ssp = Met_Drought_Frequency_Year(3).DroughtFrequency_Year.CompoundDrought;
...
h_ssp = fill([[2014:2100]';flipud([2014:2100]')],...
    ...
    RGB_ALL_Shade./255,'EdgeAlpha',0,'FaceAlpha',0.5); hold on;
clearvars -except Met_Drought_Frequency_Year Met_Drought_nat_Frequency_Year RGB_ALL_Shade RGB_ALL_Line RGB_NAT_Shade RGB_NAT_Line XlimRange YlimRange_CD YTickRange_CD
%% Plotting NAT Shade Area
% Historical CompoundDrought
CD_Historical = Met_Drought_nat_Frequency_Year(1).DroughtFrequency_Year.CompoundDrought;
...
% Scenario CompoundDrought
CD_ssp = Met_Drought_nat_Frequency_Year(2).DroughtFrequency_Year.CompoundDrought;
CD_ssp = CD_ssp - repmat((CD_ssp(:,1) - CD_Historical(:,end)),1,size(CD_ssp,2));
CD_ssp = [CD_Historical(:,end) , CD_ssp];
...
CD_piControl = Met_Drought_nat_Frequency_Year(3).DroughtFrequency_Year.CompoundDrought;
...
try
    text(1951,3.3,strcat( '[' , num2str(mean(CD_piControl),3),' ¡À ' ,...
...
try
    text(1975,3.3,strcat( '[' , num2str(mean(Change),2),' ¡À ' ,...
...
try
    ...
    Ensemble_Mean_CD_Historical = nanmean(CD_Historical([4,12,15,16,21],:))'; % Selecting 5 ESMs
    c95_CD_Historical = (std(CD_Historical([4,12,15,16,21],:))./sqrt(size(CD_Historical([4,12,15,16,21],:),1))) .* 1.96; % 95% confidence interval
    text(1951,4.3,strcat( '[' , num2str(mean(Ensemble_Mean_CD_Historical(102:151)),3),'¡À' ,...
...
try
    ...
    text(2051,4.3,strcat( '[' , num2str(mean(Ensemble_Mean_CD_ssp245(37:86)),3),'¡À' ,...
...
try
    ...
    text(2001,4.3,strcat( '[' , num2str(mean(Ensemble_Mean_CD_ssp245(152:201)),3),'¡À' ,...
...
try
    ...
    Ensemble_Mean_CD_Historical = nanmean(CD_Historical)'; % Selecting 5 ESMs
    c95_CD_Historical = (std(CD_Historical)./sqrt(size(CD_Historical,1))) .* 1.96; % 95% confidence interval
    text(1951,3.8,strcat( '[' , num2str(mean(Ensemble_Mean_CD_Historical(102:151)),3),'¡À' ,...
        num2str(mean(c95_CD_Historical(102:151)) ,3) , ']'),...
        'FontSize' , 24 , 'FontName' , 'Arial' , 'Color' , RGB_NAT_Line./255);
end
try
    % ssp 245 Mean over 2051-2100
    CD_ssp245 = Met_Drought_nat_Frequency_Year(2).DroughtFrequency_Year.CompoundDrought;
    CD_ssp245 = CD_ssp245 - repmat((CD_ssp245(:,1) - CD_Historical(:,end)),1,size(CD_ssp245,2));
    CD_ssp245 = [CD_Historical(:,end) , CD_ssp245];
    Ensemble_Mean_CD_ssp245 = nanmean(CD_ssp245)';
    c95_CD_ssp245 = (std(CD_ssp245)./sqrt(size(CD_ssp245,1))) .* 1.96; % 95% confidence interval
    text(2051,3.8,strcat( '[' , num2str(mean(Ensemble_Mean_CD_ssp245(37:86)),3),'¡À' ,...
        num2str(mean(c95_CD_ssp245(37:86)) ,3) , ']'),...
        'FontSize' , 24 , 'FontName' , 'Arial' , 'Color' , RGB_NAT_Line./255);
end
try
    ...
    text(2001,3.8,strcat( '[' , num2str(mean(Ensemble_Mean_CD_ssp245(152:201)),3),'¡À' ,...
        num2str(mean(c95_CD_ssp245(152:201)) ,3) , ']'),...
        'FontSize' , 24 , 'FontName' , 'Arial' , 'Color' , RGB_NAT_Line./255);
end
...
% setting axis
ylabel({'Frequency';'(months per decade)'});
