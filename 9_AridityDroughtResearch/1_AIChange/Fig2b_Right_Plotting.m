function Fig2b_Right_Plotting(ETrc_Year , Met_Year , Attribution_Year , Path_Fig2a_Output)
% Drawing AI Attribution calculated by PM_RC_CO2_Yang
for i_ssp = [2,3,5,4]
    ...
    if i_ssp == 4
        ...
        ETrc_Year(1).ETrc_Year.PM_RC_CO2_Yang(16,:) = [];
        Met_Year(1).Met_Year.pr(16,:) = [];
    end
    for iii = 1 : size(ETrc_Year(i_ssp).ETrc_Year.PM_RC_CO2_Yang,1)
        %% Calculate Delta AI
        ...
        AI_historical = (Met_Year(1).Met_Year.pr.*1000.*31536000./997)./...
            (ETrc_Year(1).ETrc_Year.PM_RC_CO2_Yang.*365);
        AI_ssp = (Met_Year(i_ssp).Met_Year.pr.*1000.*31536000./997)./...
            (ETrc_Year(i_ssp).ETrc_Year.PM_RC_CO2_Yang.*365);
        % 2070-2099 minus 1948-2014
        delta_AI_PM_RC_CO2_Yang(i_ssp-1 , iii) = nanmean(nanmean(AI_ssp(iii,end-30:end-1))) - nanmean(nanmean(AI_historical(iii,99:end)));
        clear AI_historical AI_ssp
        ...
        ETrc_historical = ETrc_Year(1).ETrc_Year.PM_RC_CO2_Yang; % mm/day
        ETrc_ssp = ETrc_Year(i_ssp).ETrc_Year.PM_RC_CO2_Yang; % mm/day
        ...
        rs_historical = Attribution_Year(1).Attribution_Year.rs_Yang;
        rs_ssp = Attribution_Year(i_ssp).Attribution_Year.rs_Yang;
        delta_rs = nanmean(nanmean(rs_ssp(iii,end-30:end-1))) - nanmean(nanmean(rs_historical(iii,99:end)));% 2070-2099 minus 1948-2014
        mean_rs = nanmean(nanmean([rs_historical(iii,99:end) , rs_ssp(iii,:)]));
        clear rs_historical rs_ssp
        ...
        dAI_drs_Yang(i_ssp-1 , iii) = dAI_drs(i_ssp-1 , iii);
        
        % Calculate Error of PM-RC-Yang Attribution
        Error(i_ssp-1 , iii) = delta_AI_PM_RC_CO2_Yang(i_ssp-1 , iii) -...
            sum([dAI_dpr(i_ssp-1 , iii) , dAI_dRn_s(i_ssp-1 , iii) , dAI_dVPD(i_ssp-1 , iii),...
            dAI_drs_Yang(i_ssp-1 , iii),dAI_dra(i_ssp-1 , iii),dAI_ds(i_ssp-1 , iii)]);
    end
end
%% Draw Bar
RGB_ssp_Line = [156,85,31;194,139,79;116,173,163;36,131,145];
RGB_ssp_Shade = [211,178,150;229,206,175;197,220,211;160,199,202];
Xlim_Range = [-0.1,8.6]; Ylim_Range = [-0.185,0.12];
% Plot y=0
plot(Xlim_Range,[0 0],'Color',[189,188,188]./255,'LineWidth',3);
% Plot Bar
for ii = [2,1,4,3]
    bar([0.5] , mean([delta_AI_PM_RC_CO2_Yang(ii,:)],2),...
        'FaceColor',RGB_ssp_Shade(ii,:)./255,'EdgeColor','none');
    hold on;
end
for ii = 4 : -1 : 1
    bar([2 : 8] , mean([dAI_dpr(ii,:); dAI_drs_Yang(ii,:);dAI_dRn_s(ii,:);...
        dAI_dVPD(ii,:);dAI_ds(ii,:);dAI_dra(ii,:);Error(ii,:)],2),...
        'FaceColor',RGB_ssp_Shade(ii,:)./255,'EdgeColor','none');
    hold on;
end
% set(gca,'XTick', [0.5 , 2 : 8] ,'XTickLabel',{'\Delta AI','P','rs','Rn^*','VPD','s','ra','error'})
set(gca,'XTick', [0.5 , 2 : 8] ,'XTickLabel',{''})
%% Draw ErrorBar
% Black Part of ErrorBar
X_Pos = [-0.24,-0.08,0.08,0.24];
for ii = 4 : -1 : 1
    errorbar([0.5 , 2 : 8] + X_Pos(ii) ,...
        mean([delta_AI_PM_RC_CO2_Yang(ii,:); dAI_dpr(ii,:);...
        dAI_drs_Yang(ii,:); dAI_dRn_s(ii,:); dAI_dVPD(ii,:);...
        dAI_ds(ii,:); dAI_dra(ii,:); Error(ii,:)],2) , ...
        [(std(delta_AI_PM_RC_CO2_Yang(ii,:))./sqrt(length(delta_AI_PM_RC_CO2_Yang(ii,:)))).*1.28,...
        (std(dAI_dpr(ii,:))./sqrt(length(dAI_dpr(ii,:)))).*1.28  ,...
        (std(dAI_drs_Yang(ii,:))./sqrt(length(dAI_drs_Yang(ii,:)))).*1.28  ,...
    ...
    % Ensemble Mean
    A = mean([delta_AI_PM_RC_CO2_Yang(ii,:); dAI_dpr(ii,:);...
        dAI_drs_Yang(ii,:); dAI_dRn_s(ii,:); dAI_dVPD(ii,:);...
        dAI_ds(ii,:); dAI_dra(ii,:); Error(ii,:)],2)';
    % 80% Confidence Interval
    B = [(std(delta_AI_PM_RC_CO2_Yang(ii,:))./sqrt(length(delta_AI_PM_RC_CO2_Yang(ii,:)))).*1.28,...
        (std(dAI_dpr(ii,:))./sqrt(length(dAI_dpr(ii,:)))).*1.28  ,...
        (std(dAI_drs_Yang(ii,:))./sqrt(length(dAI_drs_Yang(ii,:)))).*1.28  ,...
        (std(dAI_dRn_s(ii,:))./sqrt(length(dAI_dRn_s(ii,:)))).*1.28  ,...
        (std(dAI_dVPD(ii,:))./sqrt(length(dAI_dVPD(ii,:)))).*1.28  ,...
    ...
    if ii==4
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC_Yang' , 'B25:I25');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC_Yang' , 'B26:I26');
    elseif ii==3
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC_Yang' , 'B18:I18');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC_Yang' , 'B19:I19');
    elseif ii==2
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC_Yang' , 'B11:I11');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC_Yang' , 'B12:I12');
    else
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC_Yang' , 'B4:I4');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC_Yang' , 'B5:I5');
    end
    clear A B
end
