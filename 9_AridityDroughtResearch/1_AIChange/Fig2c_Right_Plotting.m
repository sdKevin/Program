function Fig2c_Right_Plotting(ETrc_Year , Met_Year , Attribution_Year , Path_Fig2a_Output)
% Drawing AI Attribution calculated by PM_RC
for i_ssp = [2,3,5,4]
    ...
    for iii = 1 : size(ETrc_Year(i_ssp).ETrc_Year.PM_RC,1)
        %% Calculate Delta AI
        ...
        ETrc_historical = ETrc_Year(1).ETrc_Year.PM_RC; % mm/day
        ETrc_ssp = ETrc_Year(i_ssp).ETrc_Year.PM_RC; % mm/day
        ...
        dAI_drs_PM_RC(i_ssp-1 , iii) = dAI_drs(i_ssp-1 , iii);
        
        % Calculate Error of PM-RC Attribution
        Error(i_ssp-1 , iii) = delta_AI_PM_RC(i_ssp-1 , iii) -...
...
for ii = 4 : -1 : 1
    bar([0.5 , 2 : 8] , mean([delta_AI_PM_RC(ii,:);...
        dAI_dpr(ii,:);dAI_drs_PM_RC(ii,:);dAI_dRn_s(ii,:);...
...
for ii = 4 : -1 : 1
    errorbar([0.5 , 2 : 8] + X_Pos(ii) ,...
        mean([delta_AI_PM_RC(ii,:); dAI_dpr(ii,:);...
        dAI_drs_PM_RC(ii,:); dAI_dRn_s(ii,:); dAI_dVPD(ii,:);...
        dAI_ds(ii,:); dAI_dra(ii,:); Error(ii,:)],2) , ...
        [(std(delta_AI_PM_RC(ii,:))./sqrt(length(delta_AI_PM_RC(ii,:)))).*1.28,...
        (std(dAI_dpr(ii,:))./sqrt(length(dAI_dpr(ii,:)))).*1.28  ,...
        (std(dAI_drs_PM_RC(ii,:))./sqrt(length(dAI_drs_PM_RC(ii,:)))).*1.28  ,...
    ...
    % Ensemble Mean
    A = mean([delta_AI_PM_RC(ii,:); dAI_dpr(ii,:);...
        dAI_drs_PM_RC(ii,:); dAI_dRn_s(ii,:); dAI_dVPD(ii,:);...
        dAI_ds(ii,:); dAI_dra(ii,:); Error(ii,:)],2)';
    % 80% Confidence Interval
    B = [(std(delta_AI_PM_RC(ii,:))./sqrt(length(delta_AI_PM_RC(ii,:)))).*1.28,...
        (std(dAI_dpr(ii,:))./sqrt(length(dAI_dpr(ii,:)))).*1.28  ,...
        (std(dAI_drs_PM_RC(ii,:))./sqrt(length(dAI_drs_PM_RC(ii,:)))).*1.28  ,...
        (std(dAI_dRn_s(ii,:))./sqrt(length(dAI_dRn_s(ii,:)))).*1.28  ,...
        (std(dAI_dVPD(ii,:))./sqrt(length(dAI_dVPD(ii,:)))).*1.28  ,...
    ...
    if ii==4
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC' , 'B25:I25');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC' , 'B26:I26');
    elseif ii==3
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC' , 'B18:I18');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC' , 'B19:I19');
    elseif ii==2
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC' , 'B11:I11');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC' , 'B12:I12');
    else
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , A , 'PM_RC' , 'B4:I4');
        xlswrite([Path_Fig2a_Output '80% confidence interval.xlsx'] , B , 'PM_RC' , 'B5:I5');
    end
    clear A B
end
