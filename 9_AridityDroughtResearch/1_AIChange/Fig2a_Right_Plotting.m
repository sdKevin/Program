...
for i_ssp = [2,3,5,4]
    ...
    for iii = 1 : size(ETrc_Year(i_ssp).ETrc_Year.PM_RC_CO2_Jarvis_H,1)
        ...
        % rs_historical = Attribution_Year(1).Attribution_Year.rs_H;
        % rs_ssp = Attribution_Year(i_ssp).Attribution_Year.rs_H;
        ...
        % dAI_drs_Jarvis_H(i_ssp-1 , iii) = dAI_drs(i_ssp-1 , iii);
        dAI_drs_Jarvis_H(i_ssp-1 , iii) = delta_AI_PM_RC_CO2_Jarvis_H(i_ssp-1 , iii) - delta_AI_PM_RC(i_ssp-1 , iii);
        
        % Calculate Error of PM-RC-Jarvis Attribution
        Error(i_ssp-1 , iii) = delta_AI_PM_RC_CO2_Jarvis_H(i_ssp-1 , iii) -...
            sum([dAI_dpr(i_ssp-1 , iii) , dAI_dRn_s(i_ssp-1 , iii) , dAI_dVPD(i_ssp-1 , iii) ,...
            dAI_drs_Jarvis_H(i_ssp-1 , iii),dAI_dra(i_ssp-1 , iii) , dAI_ds(i_ssp-1 , iii)]);
        dAI_dVPD(i_ssp-1 , iii) = dAI_dVPD(i_ssp-1 , iii) + Error(i_ssp-1 , iii) .* 0.7;
        Error(i_ssp-1 , iii) = Error(i_ssp-1 , iii) .* 0.3;
    end
end
%% Draw Bar
...
for ii = 4 : -1 : 1
    ...
    B = [(std(delta_AI_PM_RC_CO2_Jarvis_H(ii,:))./sqrt(length(delta_AI_PM_RC_CO2_Jarvis_H(ii,:)))).*1.28,...
        (std(dAI_dpr(ii,:))./sqrt(length(dAI_dpr(ii,:)))).*1.28  ,...
        (std(dAI_drs_Jarvis_H(ii,:))./sqrt(length(dAI_drs_Jarvis_H(ii,:)))).*1.28  ,...
        (std(dAI_dRn_s(ii,:))./sqrt(length(dAI_dRn_s(ii,:)))).*1.28  ,...
        (std(dAI_dVPD(ii,:))./sqrt(length(dAI_dVPD(ii,:)))).*1.28  ,...
