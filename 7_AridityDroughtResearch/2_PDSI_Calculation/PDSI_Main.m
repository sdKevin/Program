...
for i_GCM = 1 : length(GCM_Ensemble)
    ...
    for i_ssp = 1 : length(ssp)
            ...
            for j = 1 : n
                ...
                if nansum(isnan(data1(:,end)))==0 &&  nansum(isnan(data2_PM_RC(:,end)))==0 && isnan(awc)==0
                    ...
                    if nansum(PDSI_v_PM_RC)==0
                        pdsi_q_PM_RC(ii,j,[1:cal_mon]) = NaN;
                        pdsi_q_PM_RC_CO2_Jarvis_H(ii,j,[1:cal_mon]) = NaN;
                        pdsi_q_PM_RC_CO2_Yang(ii,j,[1:cal_mon]) = NaN;
                    else
        ...
        if i_ssp == 1
            ...
            save(strcat(OutputPath{1} , GCM) ,...
                'pdsi_PM_RC' , 'pdsi_PM_RC_CO2_Jarvis_H' , 'pdsi_PM_RC_CO2_Yang');
            clear pdsi pdsi_q Pr PET_PM_RC PET_PM_RC_CO2_Jarvis_H PET_PM_RC_CO2_Yang
        else
            save(strcat(OutputPath{ssp(i_ssp) + 1} , GCM) ,...
                'pdsi_PM_RC' , 'pdsi_PM_RC_CO2_Jarvis_H' , 'pdsi_PM_RC_CO2_Yang');
            clear pdsi_PM_RC pdsi_PM_RC_CO2_Jarvis_H pdsi_PM_RC_CO2_Yang
    ...
    clear ssp GCM Pr_hist PM_RC_hist PM_RC_CO2_Jarvis_H_hist PM_RC_CO2_Yang_hist
    clear i_ssp pdsi_q_PM_RC pdsi_q_PM_RC_CO2_Jarvis_H pdsi_q_PM_RC_CO2_Yang
end
