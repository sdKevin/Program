    ...
    for i_ssp = 1 : length(ssp)
        ...
        for ii = 1 : m
            ...
            for j = 1 : n
                ...
                % detrend P following Zhang et al., 2015, GRL
                data1_Year = sum(reshape(data1,12,[]));
                data1_detrend_Year = detrend(sum(reshape(data1,12,[])));
                data1_ratio = (data1_detrend_Year + (data1_Year(1) - data1_detrend_Year(1))) ./ data1_Year; clear data1_Year;
                for Pos = 1 : length(data1)
                    data1_detrend(Pos,1) = data1(Pos) .* data1_ratio(ceil(Pos/12));
                    if data1_detrend(Pos,1)<0
                        data1_detrend(Pos,1)=0;
                    end
                end
                ...
                if nansum(isnan(data1_detrend(:,end)))==0 && nansum(isnan(data2_PM_RC(:,end)))==0 && isnan(awc)==0
                    % Input data includes [year,month,monthly potential evapotranspiration,
                    % monthly precipitation,available water capacity]
                    DATA_PM_RC = [DateGens(Year) , data2_PM_RC , data1_detrend , awc*ones(cal_mon,1)];
                    DATA_PM_RC_CO2_Jarvis_H = [DateGens(Year) , data2_PM_RC_CO2_Jarvis_H , data1_detrend , awc*ones(cal_mon,1)];
                    DATA_PM_RC_CO2_Yang = [DateGens(Year) , data2_PM_RC_CO2_Yang , data1_detrend , awc*ones(cal_mon,1)];
                    % Calculate PDSI.
        ...
        pdsi_deTrend_PM_RC = pdsi_q_PM_RC(: , : , [1981:end]);
        pdsi_deTrend_PM_RC_CO2_Jarvis_H = pdsi_q_PM_RC_CO2_Jarvis_H(:,:,[1981:end]);
        pdsi_deTrend_PM_RC_CO2_Yang = pdsi_q_PM_RC_CO2_Yang(:,:,[1981:end]);
        
        scpdsi_deTrend_PM_RC = scpdsi_q_PM_RC(: , : , [1981:end]);
        scpdsi_deTrend_PM_RC_CO2_Jarvis_H = scpdsi_q_PM_RC_CO2_Jarvis_H(:,:,[1981:end]);
        scpdsi_deTrend_PM_RC_CO2_Yang = scpdsi_q_PM_RC_CO2_Yang(:,:,[1981:end]);
        if i_ssp == 1
            % save PDSI for future period
            save(strcat(OutputPDSIPath{ssp(i_ssp) + 1} , 'PdeTrend_' , GCM) ,...
                'pdsi_deTrend_PM_RC' , 'pdsi_deTrend_PM_RC_CO2_Jarvis_H' , 'pdsi_deTrend_PM_RC_CO2_Yang');
            clear pdsi_deTrend_PM_RC pdsi_deTrend_PM_RC_CO2_Jarvis_H pdsi_deTrend_PM_RC_CO2_Yang
            % save scPDSI for future period
            save(strcat(OutputscPDSIPath{ssp(i_ssp) + 1} , 'PdeTrend_' , GCM) ,...
                'scpdsi_deTrend_PM_RC' , 'scpdsi_deTrend_PM_RC_CO2_Jarvis_H' , 'scpdsi_deTrend_PM_RC_CO2_Yang');
            clear scpdsi_deTrend_PM_RC scpdsi_deTrend_PM_RC_CO2_Jarvis_H scpdsi_deTrend_PM_RC_CO2_Yang
            
            % save PDSI and scPDSI for historical period
            pdsi_deTrend_PM_RC = pdsi_q_PM_RC(: , : , [1:1980]);
            pdsi_deTrend_PM_RC_CO2_Jarvis_H = pdsi_q_PM_RC_CO2_Jarvis_H(: , : , [1:1980]);
            pdsi_deTrend_PM_RC_CO2_Yang = pdsi_q_PM_RC_CO2_Yang(: , : , [1:1980]);
            
            scpdsi_deTrend_PM_RC = scpdsi_q_PM_RC(: , : , [1:1980]);
            scpdsi_deTrend_PM_RC_CO2_Jarvis_H = scpdsi_q_PM_RC_CO2_Jarvis_H(: , : , [1:1980]);
            scpdsi_deTrend_PM_RC_CO2_Yang = scpdsi_q_PM_RC_CO2_Yang(: , : , [1:1980]);
            
            save(strcat(OutputPDSIPath{1} , 'PdeTrend_' , GCM) ,...
                'pdsi_deTrend_PM_RC' , 'pdsi_deTrend_PM_RC_CO2_Jarvis_H' , 'pdsi_deTrend_PM_RC_CO2_Yang');
            clear pdsi_deTrend_PM_RC pdsi_deTrend_PM_RC_CO2_Jarvis_H pdsi_deTrend_PM_RC_CO2_Yang
            save(strcat(OutputscPDSIPath{1} , 'PdeTrend_' , GCM) ,...
                'scpdsi_deTrend_PM_RC' , 'scpdsi_deTrend_PM_RC_CO2_Jarvis_H' , 'scpdsi_deTrend_PM_RC_CO2_Yang');
            clear scpdsi_deTrend_PM_RC scpdsi_deTrend_PM_RC_CO2_Jarvis_H scpdsi_deTrend_PM_RC_CO2_Yang
